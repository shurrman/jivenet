...
DoH-цепочка по типу активной сети — конец ping-pong'а на сотовой РФ.

### Android — Исправлено

- **Watchdog бесконечно цикличил `cellular ↔ public-DoH`** на сотовой
  МегаФон/MTS/Билайн. Корень: до 0.9.7 список DoH был всегда
  `[cellularDNS, cfg.doh]` независимо от текущей сети. На сотовой РФ
  оператор режет TCP/443 ко всем публичным DoH (1.1.1.1, 8.8.8.8 и т.д.):
  ```
  sendLoop: Post "https://1.1.1.1/dns-query": dial tcp 1.1.1.1:443: connect: connection refused
  ```
  Cellular DoH работал, но иногда «затыкался» на 15+ секунд — watchdog
  расценивал это как STALL и переключался на fallback, который мгновенно
  дох. Через 60с возвращался обратно. И так бесконечно.

  Теперь `DohChain.buildList()` строит список **по типу активной сети**:
  - **На сотовой** (есть TRANSPORT_CELLULAR + NOT_VPN, **нет** Wi-Fi/Ethernet)
    → list = `[cellular_dns_оператора]`. Один элемент → watchdog не
    запускается (`if (dohList.size > 1)` в startTunnel), никаких
    переключений. Оператор всё равно режет публичные DoH — нет смысла
    держать fallback.
  - **На Wi-Fi/Ethernet** → list = `[cfg.doh]` (пользовательский,
    обычно Cloudflare/Google). Cellular DNS бесполезен — не
    маршрутизируется через wlan0.

  Смена типа сети (Wi-Fi ↔ Cellular) триггерит auto-reconnect через
  существующий `NetworkCallback` → `restartTunnel()` пересобирает
  список под новый интерфейс. Watchdog теперь активируется только когда
  список действительно содержит ≥2 разных DoH (редкий случай — например
  если в будущем добавим несколько публичных в Wi-Fi-сценарии).

  Поле-проверено на МегаФон LTE: с одним cellular DoH туннель работает
  стабильно, ifconfig.co показывает 93.77.166.152, watchdog тих.
