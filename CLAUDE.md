...
1. **`CHANGELOG.md`** — добавить новую секцию `## X.Y.Z — YYYY-MM-DD` сверху
   с разделами «Добавлено / Изменено / Исправлено». **Только то, что
   изменилось с предыдущей версии** — не повторять ранее уже описанные
   фичи. Описывать кратко но с конкретикой (какой код / что фиксит /
   поле-проверено или нет).
   Файл `CHANGELOG.md` целиком содержит ВСЮ историю (от первой версии
   до текущей) — каждая секция = diff vs предыдущая.
   На странице GitHub Release `scripts/onboard.sh` через awk выдёргивает
   только секцию текущей версии + ссылку на полный CHANGELOG в репо
   (иначе юзер скроллил бы до v0.1.0 и читал бы исторические
   ограничения как «текущие»).
